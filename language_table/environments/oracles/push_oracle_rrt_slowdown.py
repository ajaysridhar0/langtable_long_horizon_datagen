...
class ObstacleOrientedPushOracleBoard2dRRT(
  ...
  def __init__(self,
    ...
    self._slowdown_exponent = float(slowdown_exponent)
  ...
  def _maybe_slowdown(self, dist, xy_delta):
    ...
    for (thresh, slowdown) in zip(dist_thresh, slowdowns):
      if dist < thresh:
        exponent = max(0.0, self._slowdown_exponent)
        # exponent=0 => no slowdown (factor=1). exponent=1 => baseline.
        xy_delta *= slowdown ** exponent
        break
    return xy_delta
