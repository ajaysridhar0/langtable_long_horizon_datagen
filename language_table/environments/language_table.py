...
class LanguageTable(gym.Env):
  ...
  def __init__(self,
    ...
    Args:
      ...
      render_backend: str, Renderer to use for camera images ("tiny" or
        "hardware"). "hardware" requires the EGL plugin in headless mode.
      image_width: Optional int, override for camera image width in pixels.
      image_height: Optional int, override for camera image height in pixels.
    """
    ...
    # Optional: avoid spawning blocks inside their goal regions.
    self._avoid_goal_regions = False
    self._avoid_goal_region_margin = None
    self._combo_idx_override = None
    self._last_combo_idx = None
  ...
  def set_combo_index_override(self, combo_idx: Optional[int]):
    """Pins block-combination sampling to a deterministic index for reset()."""
    ...
    if combo_idx < 0:
      self._combo_idx_override = None
      return
    self._combo_idx_override = combo_idx
  ...
  def reset(self, reset_poses = True):
    ...
    if self._combo_idx_override is None:
      combo_idx = self._rng.choice(range(len(all_combinations)))
    else:
      combo_idx = int(self._combo_idx_override) % len(all_combinations)
    self._last_combo_idx = int(combo_idx)
  ...
  def _render_camera(self, image_size, return_seg_mask=False):
    ...
    _, _, color, _, _ = self._pybullet_client.getCameraImage(
        ...
        renderer=(pybullet.ER_BULLET_HARDWARE_OPENGL
                  if self._render_backend == "hardware"
  ...
  def _setup_pybullet_scene(self):
    ...
    if self._render_backend == "hardware":
      try:
        plugin_id = self._pybullet_client.loadPlugin("eglRendererPlugin")
        if plugin_id < 0:
          raise RuntimeError("eglRendererPlugin returned negative id")
        logging.info("Loaded EGL renderer plugin (id=%s).", plugin_id)
      except Exception as exc:
        logging.warning(
            "Failed to load EGL renderer plugin; falling back to tiny renderer: %s",
            exc,
        )
        self._render_backend = "tiny"
  ...
  def _reset_poses_randomly(self, blocks_on_table):
    ...
    def _in_location_region(block_xy, location, margin):
      if not location:
        return False
    ...
    id_to_block = {v: k for k, v in self._block_to_pybullet_id.items()}
    color_to_corner_map = None
    region_margin = None
    if self._avoid_goal_regions and self._reward_calculator is not None:
      color_to_corner_map = getattr(self._reward_calculator, "_color_to_corner_map", None)
      region_margin = self._avoid_goal_region_margin
      if region_margin is None:
        region_margin = getattr(self._reward_calculator, "_region_margin", 0.0)
    ...
    while True:
      ...
      for block_id in visible_block_ids:
        ...
        while True:
          ...
          if self._avoid_goal_regions and color_to_corner_map is not None:
            block_name = id_to_block.get(block_id, "")
            color = block_name.split("_")[0] if block_name else ""
            location = color_to_corner_map.get(color)
            if location and _in_location_region(candidate_translation[:2], location, region_margin):
              num_attempts += 1
